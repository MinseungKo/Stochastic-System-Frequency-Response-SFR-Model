...
sims = 500; % Total simulation runs
...
for sim=1:sims
    ...
    dcorrload = vloadgen(vsens_rand, dgenvolts, genvolts, dt);    
...
function dcorrload=vloadgen(vsens, dgenvolts, genvolts, dt)
    for i=1:size(genvolts,1)
        calcgenvolt(i,:) = vsens(:,30:end)'*dgenvolts(i,:)';
        ratios = dgenvolts(i,:)./calcgenvolt(i,:);
        ratios(isnan(ratios))=0;
    
        for j=1:size(ratios,2)
            loadvsens(j,:) = vsens(j,1:29)*ratios(j);
            loadvsens(isnan(loadvsens))=0;
        end
        
        if i>1
            ddgenvolts = dgenvolts(i,:)-dgenvolts(i-1,:);
            calcdgenvolt(i,:) = vsens(:, 30:end)'*ddgenvolts';
            dratios = ddgenvolts./calcdgenvolt(i,:);
            dratios(isnan(dratios))=0;
    
            for j=1:size(dratios,2)
                loaddvsens(j,:) = vsens(j,1:29)*dratios(j);
            end
        else
            loaddvsens=zeros(size(vsens(1,1:29)));
        end
        
        if i==1
            dcorrload(i,:) = loadvsens'*dgenvolts(i,:)';
        else
            addfac = (loaddvsens'*(dgenvolts(i,:)-dgenvolts(i-1,:))');
            addfac2 = loadvsens'*dgenvolts(i,:)';
            for j=1:29
                if i*dt<3
                    dcorrload(i,j) = dcorrload(i-1,j)+addfac(j);
                else 
                    dcorrload(i,j) = addfac2(j);
                    % dcorrload(i,:) = vsens(:,1:29)'*dgenvolts(i,:)';
                end
            end
        end
        if sum(isnan(dcorrload))>0
            display('wrong')
            break
        end
    end
